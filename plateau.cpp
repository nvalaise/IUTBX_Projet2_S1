...
void initPlateau(Plateau &plateau)
{
	...
	for (int i = 0; i < NB_BLOCK_HAUTEUR; i++)
	{
		for (int j = 0; j < NB_BLOCK_LARGEUR; j++)
		{
            ...
            if((i==3) && (j==3))
            {
                plateau.position[i][j].valeur = 2;
            }
			else if (nombreAleatoire == 1 && nbPieceDix < 14 && (i != 3 || j != 3) && (plateau.matrice[i][j-1].valeur != 10 || plateau.matrice[i][j-2].valeur != 10))
			{
				nbPieceDix++;
...
void afficherPlateau(Plateau &plateau, Gestion &jeu)
{
	for (int i = 0; i < NB_BLOCK_HAUTEUR; i++)
	{
		for (int j = 0; j < NB_BLOCK_LARGEUR; j++)
		{
			...
			else if (plateau.position[i][j].valeur == 2)
			{
